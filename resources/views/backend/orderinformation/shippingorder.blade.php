   ...
   <div class=" col-sm-12">
    ...
    <div class="card-body" style="overflow-x:auto;">
     ...
     <tbody>
      ...
      @foreach($ShippingOrder as $ShippingOrderShow)
      <tr>
        <td>
          <a href="{{ url('invoice/'.$ShippingOrderShow->id)}}" class="text-dark">{{ $ShippingOrderShow->id}}</a>
        </td>
        <td>
          {{ $ShippingOrderShow->order_date}}
        </td>
        <td>
          {{ $ShippingOrderShow->name}}
        </td>
        <td>
          {{ $ShippingOrderShow->phone}}
        </td>
        <td>
          {{ $ShippingOrderShow->email}}
        </td>
        <td>
          {{ $ShippingOrderShow->payment_method}}
        </td>
        <td>
          @if($ShippingOrderShow->status==0)
          <span class="orderstatus bg-dark">Pending</span>
          @elseif($ShippingOrderShow->status==1)
          <span class="orderstatus1 bg-primary">Processing</span>
          @elseif($ShippingOrderShow->status==2)
          <span class="orderstatus2 bg-success">Shipping</span>
          @elseif($ShippingOrderShow->status==3)
        ...
        <td>
          <form method="POST" class="btn-submit" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="" id="id" value="{{ $ShippingOrderShow->id }}">
